...
//! be written into it, then [`snapshot`] on top of it, then [`wal`], then [`recovery`] joining the
//! two, and [`collection`] putting the ordering rule somewhere nobody has to remember it.
...
pub use collection::{Collection, Opened};
